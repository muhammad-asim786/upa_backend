 ...
 */
export const registerUser = async (email, firstName, lastName, phoneNumber, password) => {
  try {
    ...
    const user = new User({
      ...
      passwordHash,
      isVerified: true, // User is verified since they provided password during registration
    });
    ...
    const { token: refreshToken, expiresAt: refreshTokenExpiresAt } = await createRefreshToken(user._id);
